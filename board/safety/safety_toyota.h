...
static bool toyota_tx_hook(const CANPacket_t *to_send) {
  ...
  if (addr == 0x750) {
    ...
    // Enhanced BSM
    bool sp_invalid_uds_msgs = (GET_BYTES(to_send, 0, 4) != 0x10002141) ||
                               (GET_BYTES(to_send, 0, 4) != 0x60100241) ||
                               (GET_BYTES(to_send, 0, 4) != 0x69210241);

    sp_invalid_uds_msgs |= (GET_BYTES(to_send, 0, 4) != 0x10002142) ||
                           (GET_BYTES(to_send, 0, 4) != 0x60100242) ||
                           (GET_BYTES(to_send, 0, 4) != 0x10002142) ||
                           (GET_BYTES(to_send, 0, 4) != 0x69210242);

    sp_invalid_uds_msgs |= (GET_BYTES(to_send, 0, 4) != 0x11300540);
