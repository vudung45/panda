...
static bool toyota_tx_hook(const CANPacket_t *to_send) {
  ...
  if (addr == 0x750) {
    ...
    // Enhanced BSM
    bool sp_invalid_uds_msgs = (GET_BYTES(to_send, 0, 4) != 0x10002141) ||  // disable left BSM debug
                               (GET_BYTES(to_send, 0, 4) != 0x60100241) ||  // enable left BSM debug
                               (GET_BYTES(to_send, 0, 4) != 0x69210241) ||  // poll left BSM status
                               (GET_BYTES(to_send, 0, 4) != 0x10002142) ||  // disable right BSM debug
                               (GET_BYTES(to_send, 0, 4) != 0x60100242) ||  // enable right BSM debug
                               (GET_BYTES(to_send, 0, 4) != 0x69210242);    // poll right BSM status

    if (invalid_uds_msg && sp_invalid_uds_msgs) {
      tx = false;
    }
  }
